...
async def login(form_data: OAuth2PasswordRequestForm = Depends(), db: Session = Depends(get_db)):
    ...
    if not user.hashed_password or user.hashed_password == "GOOGLE_OAUTH_NO_PASSWORD":
        raise HTTPException(
            status_code=status.HTTP_401_UNAUTHORIZED,
...
async def google_login(data: GoogleAuthRequest, db: Session = Depends(get_db)):
    ...
    Creates the user if they don't exist, then returns our own JWT.
    """
    user = db.query(UserModel).filter(UserModel.email == data.email).first()

    if not user:
        # First time — create a new account (no password)
        user = UserModel(
            email=data.email,
            name=data.name,
        ...
        # Update avatar if changed, but protect custom local gallery uploads
        if data.avatar_url and user.avatar_url != data.avatar_url:
            is_custom = user.avatar_url and user.avatar_url.startswith("/uploads")
            if not is_custom:
